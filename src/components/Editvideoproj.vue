...
export default {
  ...
  data() {
    return {
      ...
      labeledtime: new Date().getTime().toString()
    }
  },
  mounted() {
    getvideofiles(this.$store.state.currentprojid, this.$store.state.video).then(value => {
      for(var i = 0; i < value.data.length; i++){
        let tmp = require('../../../projects/videofiles/' + this.$store.state.videoname + '/' + value.data[i].pic_name);
        console.log(tmp)
        this.pics.push({cropImage: tmp});
        this.pic_name.push(value.data[i].pic_name);
      }
  ...
  computed: {
    ...
    taskorproj: function (){
      if(this.$store.state.currenttype === 'project'){
        return true;
      }
      else{
        return false;
